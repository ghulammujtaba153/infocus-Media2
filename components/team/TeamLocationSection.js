...
const TeamLocationSection = () => {
  return (
    <section className="bg-white px-6 lg:px-6 flex flex-col lg:flex-row items-center justify-between gap-6 pt-16 md:pt-24 lg:pt-32 pb-8">
    {/* Text Section */}
    <div className="flex-1 flex flex-col justify-between bg-black text-black px-8 w-full min-h-screen lg:h-screen lg:w-1/2">
      ...
      <div className="text-[22px]  lg:w-[728]  leading-relaxed mb-20 mt-20 md:mt-0">
        ...
        <div className="pl-2 sm:pl-4 md:pl-8 lg:pl-10 mt-10">
          <Link href="/careers">
            <button className="bg-white rounded-md cursor-pointer text-black text-[14px] sm:text-[16px] md:text-[18px] lg:text-[22px] px-4 sm:px-6 md:px-10 lg:px-12 py-2 sm:py-3 pl-2 sm:pl-4 md:pl-8 lg:pl-12 hover:scale-105 transition-transform duration-300  font-medium mt-4  max-w-xs sm:max-w-sm md:max-w-md lg:max-w-lg">
              Join the Team
            </button>
          </Link>
        </div>
    ...
    <div className="flex-1 w-full lg:w-1/2 relative overflow-hidden min-h-screen lg:h-screen">
      {/* Address Overlay */}
      ...
      <a href="https://maps.app.goo.gl/cWRBwXFomTUYaeYs9" target="_blank" rel="noopener noreferrer">
        <img src="/map_home.png" className="w-full h-screen object-cover" alt="map" />
      </a>
    </div>
  </section>
  );
